...
export let Case = (props) => {
    ...
    useEffect(() => {
        if (props.mooving === 'left') {
            ...
            setLay('');
            (props.caseIndex === props.indexCenter) ? (setOpacity(style.opaciter)) : setOpacity(style.none_opaciter);
            console.log(Math.abs((props.indexCenter - props.caseIndex)) * -1 + 5);
    ...
    return (
        <div className={style.for_case + ' ' + layout}>
            <div ref={Card} style={{ zIndex: Math.abs((props.indexCenter - props.caseIndex)) * -1 + 5, transform: 'scale(' + (1 - Math.abs((props.indexCenter - props.caseIndex)) * 0.1) + ')', backgroundImage: 'url(' + props.photo + ')', marginLeft: (props.indexCenter - props.caseIndex) * 30 + 5 * (props.indexCenter - props.caseIndex) + '%' }} className={style.case + ' ' + opaciter}></div>
        </div>
    )
}